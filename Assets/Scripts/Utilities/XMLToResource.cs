...
public static class XMLToResource 
{
    ...
    public static void LoadWeaponData()
    {
        ...
        filePath += "weapon_database.xml";

        if(!File.Exists(filePath))
        {
            Debug.Log("weapon_database.xml doesnt exist! Aborting");
            return;
        }
        ...
        if(obj == null)
        {
            Debug.Log("WeaponScriptableObject doesnt exist! Aborting.");
            return;
        }
    ...
    static void XmlToActions(XmlDocument doc,string nodeName, ref Weapon _w)
    {
        foreach(XmlNode a in doc.DocumentElement.SelectNodes("//" + nodeName))
        {
            ...
            XmlNode overideDamageAnim = a.SelectSingleNode("overideDamageAnim");
            _a.overideDamageAnim = (overideDamageAnim.InnerText == "True");
